...
g_user_i = 0
g_proxy_i = -1
g_max_switch_time = 100000000

def switchConf(u=False, p=False):
    """
...
def locateCookieFile(username, cookieStoreBase):
    return cookieStoreBase+'.'+username + '.dat'

def initGF(force=False):
    global GF
    userConf, proxy = switchConf(True, True)
    ...
    while True:
        ...
        if status == True:
            return True
        if aSwitched:
            logging.error('call Switch and relogin into switch conf %s|%s' % (username, passwd))
        ...
        if c > max_times:   
            ...
            userConf, proxy = switchConf(u=True, p=True)
        time.sleep(10)
        logging.error('after switch conf %s|%s' % (username, passwd))
        username, passwd = userConf
        cookieFile = locateCookieFile(username, conf.cookieStoreBaseWap)
        if not aSwitched:
            logging.error('call Switch into switch conf %s|%s' % (username, passwd))
...
def getUserContent(user, pages=10):
    ...
    nextUrl = userUrl
    c = 0
    while c < pages and nextUrl != None:
        c += 1
        data, reUrl = requestPage(userUrl)
    infoUrl = findUserInfoUrl(data)
    if infoUrl == None:
