...
class LTML(object):
    ...
    def get_words_and_pos(self, pid, sid, encoding=None):
        ret = [(e.get('cont'), e.get('pos')) for e in self.dom[1][pid][sid].findall('word')]

        if encoding is None:
            return ret
        else:
            return [(r[0].encode(encoding), r[1])for r in ret]
